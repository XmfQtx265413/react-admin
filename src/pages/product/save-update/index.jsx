...
class SaveUpdate extends Component {
  ...
  getCategories = async (parenId) => {
    const result = await reqCategories(parenId);
    if (result) {
      if (parenId === '0') {
        this.setState({
  ...
  loadData = async selectedOptions => {
    console.log(selectedOptions); //选中的一级分类数据
    //获取数组最后一项
    ...
    if (result) {
      //请求成功将loading图改为false
      targetOption.loading = false;
      //targetOption.children表示二级分类
  ...
  componentWillUnmount() {
    console.log('saveupdate -- componentWillUnmount()');
  }
